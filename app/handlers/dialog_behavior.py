...
async def command_anek(message: types.Message):
    ...
    await __main__.bot.send_message(chat_id=message.chat.id, text=text)
...
async def message_processing_mainchat(message: types.Message):
    ...
    active.models['MainChat'].parse_and_add(text=message.text)
    text = active.models['MainChat'].generate_answer(message=message.text)
    await __main__.bot.send_message(chat_id=message.chat.id, text=text)
...
def register_message_handlers(dp: Dispatcher):
    ...
    dp.register_message_handler(message_processing_mainchat, filters.IDFilter(chat_id=-1001446215055))
